...
def post_request(userName, password, terminate_flag):
    global token
...
def worker(userName, passwordFile, terminate_flag):
    global cookie        
    userName = userName
    cookie = {}
    get_request()
    ...
    with open(passwordFile, 'r') as file:
        for idx, line in enumerate(file, start=1):
            if(not terminate_flag.value):
                password = line.strip()
                print("{0} Checking {1} : {2}".format(idx, userName, password))
                post_request(userName, password,terminate_flag)
            else:
                break
...
if __name__ == '__main__':
    ...
    for passwordFile in filenames:
        process = multiprocessing.Process(target=worker, args=(userName, passwordFile, terminate_flag))
